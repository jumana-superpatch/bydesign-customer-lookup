    ...
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            if (byDesignCustomer) {
                ...
                // 3.5 Add addresses (Shopify requires separate mutation)
                const addressesCreated = [];
                if (createdCustomer?.id) {
                    // Billing address
                    if (byDesignCustomer.BillStreet1) {
                    ...
                    // Shipping address
                    if (byDesignCustomer.ShipStreet1 && byDesignCustomer.ShipStreet1 !== byDesignCustomer.BillStreet1) {
...
async function findCustomerInShopify(email, shop, token) {
    ...
    return json.data?.customers?.edges?.node || null;
}
...
// create customer in Shopify (basic info only)
...
// create address in Shopify (for given customer ID)
async function createCustomerAddressInShopify(customerId, address, shop, token) {
    const mutation = `mutation customerAddressCreate($customerId: ID!, $address: MailingAddressInput!) {
